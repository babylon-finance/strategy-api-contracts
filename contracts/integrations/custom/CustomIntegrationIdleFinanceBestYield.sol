...
import {IIdleToken} from '../../interfaces/external/idle-finance/IIdleToken.sol';
import {IIdlePriceHelper} from '../../interfaces/external/idle-finance/IIdlePriceHelper.sol';
 ...
 */
contract CustomIntegrationIdleFinanceBestYield is CustomIntegration {
    ...
    IIdlePriceHelper private immutable idlePriceHelper;
     ...
     */
    constructor(IBabController _controller, IIdlePriceHelper _idlePriceHelper) CustomIntegration('custom_idle_finance_best_yield', _controller) {
        require(address(_controller) != address(0), 'invalid address');
        idlePriceHelper = _idlePriceHelper;
    }
    ...
    function _isValid(
        ...
        return _isValidIdleVault(BytesLib.decodeOpDataAddressAssembly(_data, 12));
    }
        ...
        )
    {
        ...
        require(_isValidIdleVault(vault), 'Idle.Finance vault is not valid');
        bytes memory methodData = abi.encodeWithSelector(
          IIdleToken.mintIdleToken.selector,
          _maxAmountsIn[0],
          false, // _skipWholeRebalance
          address(0) //_referral
        );
    ...
    {
        address vault = BytesLib.decodeOpDataAddress(_data);
        bytes memory methodData = abi.encodeWithSelector(IIdleToken.redeemIdleToken.selector, _resultTokensIn);
    ...
    function getInputTokensAndWeights(
        ...
        inputTokens[0] = IIdleToken(vault).token();
    ...
    function getOutputTokensAndMinAmountOut(
        ...
        outputTokens[0] = IIdleToken(vault).token();
        uint256[] memory outputAmounts = new uint256[](1);
        outputAmounts[0] = IIdleToken(vault).tokenPriceWithFee(_strategy).preciseMul(_liquidity);
    ...
    function getPriceResultToken(
        ...
        address vault = BytesLib.decodeOpDataAddress(_data);
        return IIdleToken(vault).tokenPrice().preciseMul(
          _getPrice(IIdleToken(vault).token(), _tokenDenominator)
        );
    }
    ...
    function _getRewardTokens(
      ...
      return IIdleToken(_data).getGovTokens();
    }

    function _isValidIdleVault(
        address _vaultAddress
    ) internal view returns (bool) {
      try idlePriceHelper.getRedeemPrice(_vaultAddress) returns (uint price) {
        return price > 0;
      } catch (bytes memory) {
        return false;
      }
